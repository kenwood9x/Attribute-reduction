...
if __name__ == "__main__":
    ...
    W = []
    data = DS_split[0]
    # chay lan 1, voi phan du lieu dau tien (tinh)
    F1 = IntuitiveFuzzy(data, att_nominal_cate, np.vstack((data, DS_split[1])))
    W1 = F1.filter(verbose=False)
    W.append(W1)
    print(W1)
    print(F1.evaluate(W1,10))
    # chay cac buoc tiep theo, du lieu gia tang
    for i in range(1, len(DS_split)):
        Fx = IntuitiveFuzzy(data, att_nominal_cate, np.vstack((data, DS_split[i])))
        #Wx = Fx.remove_object()
        Wx = Fx.filter_icr(verbose=False)
        print(Wx)
