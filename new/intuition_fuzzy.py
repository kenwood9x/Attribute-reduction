...
class IntuitiveFuzzy(object):
    ...
    def _get_ro(self,icr = True):
        if icr==True:
            n_class = self.num_class_icr
        else: 
    ...
    def filter_icr(self,B, verbose=False):
        """
            The main function for the filter phase
            Params :
                - verbose : Show steps or not
        ...
        matrix_B = reduce(self._get_union_IFRM,[self.relational_matrices[x] for x in B])
        matrix_B_icr = reduce(self._get_union_IFRM,[self.relational_matrices_icr[x] for x in B])
        W = []
        d = self.dist_d_icr(matrix_B, matrix_B_icr)
        
        D = (self.dist_d_icr_st1(self.relational_matrices_distance["C"], matrix_C_icr) )
        ro = self._get_ro(icr = True)
